...
object BitmapUtil {
    ...
    private fun fetchPicture(context: Context, media: MediaWrapper): Bitmap? {

        val picture = readCoverBitmap(context, media.artworkURL)
        if (picture != null) BitmapCache.addBitmapToMemCache(media.location, picture)
        return picture
    }

    fun getPicture(context: Context, media: MediaWrapper): Bitmap? {
        val picture = getPictureFromCache(media)
        return picture ?: fetchPicture(context, media)
    }

    private fun readCoverBitmap(context: Context, path: String?): Bitmap? {
