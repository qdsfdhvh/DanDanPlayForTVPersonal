...
internal fun <T> DiffUtil.ItemCallback<T>.toDiffCallback(): DiffCallback<T> {
    return object : DiffCallback<T>() {
        override fun areItemsTheSame(oldItem: T, newItem: T) = this@toDiffCallback.areItemsTheSame(oldItem, newItem)
        override fun areContentsTheSame(oldItem: T, newItem: T) = this@toDiffCallback.areContentsTheSame(oldItem, newItem)
        override fun getChangePayload(oldItem: T, newItem: T): Any? = this@toDiffCallback.getChangePayload(oldItem, newItem)
    }
}