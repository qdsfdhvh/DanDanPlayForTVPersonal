...
//            return if (hasCache) StringBuilder(cacheDir!!).append('/').append(item.fileName).append(".jpg").toString() else null
    ...
    suspend fun obtainBitmap(item: MediaLibraryItem, width: Int) = withContext(Dispatchers.IO) {
        when (item) {
            ...
            else -> readCoverBitmap(
                Uri.decode(item.artworkMrl),
                width
            )
        }
    }
...
//        if (hasCache && File(thumbPath).exists()) return readCoverBitmap(thumbPath, width)
    ...
    private fun composeImage(mediaList: List<MediaWrapper>, imageWidth: Int): Bitmap? {
        ...
        return if (count == 1) sourcesImages[0] else composeCanvas(
            sourcesImages.filterNotNull().toTypedArray(),
            count,
            minWidth,
            minHeight
        )
    ...
    @WorkerThread
    private fun getComposedImage(key: String, mediaList: List<MediaWrapper>, width: Int): Bitmap? {
    ...
    @WorkerThread
    private fun getFolderThumbnail(folder: Folder, width: Int): Bitmap? {
    ...
    @WorkerThread
    private fun getVideoGroupThumbnail(group: VideoGroup, width: Int): Bitmap? {
